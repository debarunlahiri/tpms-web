...
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forgot Password | TPMS</title><script src="https://cdn.tailwindcss.com"></script><script>tailwind.config={theme:{extend:{colors:{primary:{500:'#3b82f6',600:'#2563eb',700:'#1d4ed8'},secondary:{900:'#0f172a'}},animation:{'slide-up':'slideUp .8s ease-out','float':'float 6s ease-in-out infinite','pulse-slow':'pulse 4s cubic-bezier(.4,0,.6,1) infinite'},keyframes:{slideUp:{'0%':{opacity:'0',transform:'translateY(30px)'},'100%':{opacity:'1',transform:'translateY(0)'}},float:{'0%, 100%':{transform:'translateY(0)'},'50%':{transform:'translateY(-20px)'}}}}}}</script><link rel="stylesheet" href="assets/css/style.css"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"></head>
<body class="bg-secondary-900 min-h-screen flex items-center justify-center overflow-hidden relative p-6"><div class="absolute inset-0 overflow-hidden"><div class="absolute -top-40 -left-40 w-96 h-96 bg-primary-600/20 rounded-full blur-3xl animate-float"></div><div class="absolute top-1/2 -right-40 w-80 h-80 bg-purple-600/20 rounded-full blur-3xl animate-float" style="animation-delay:2s"></div><div class="absolute -bottom-40 left-1/3 w-72 h-72 bg-pink-600/20 rounded-full blur-3xl animate-float" style="animation-delay:4s"></div></div>
<main class="relative z-10 w-full max-w-md bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-8 animate-slide-up"><div class="text-center mb-8"><div class="w-36 h-16 mx-auto rounded-xl bg-white overflow-hidden flex items-center justify-center shadow-lg mb-4 animate-pulse-slow"><img src="assets/images/logo.png" alt="TPMS" class="w-full h-full object-cover scale-[1.6]"></div><h1 class="text-2xl font-bold text-white mb-1"><?php echo $pageHeading; ?></h1><p class="text-gray-400 text-sm"><?php echo $pageText; ?></p></div>

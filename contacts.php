    ...
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        try {
            if (isset($_POST['id']) && $_POST['id']) {
                ...
                $stmt = $db->prepare("UPDATE contacts SET first_name=?, last_name=?, email=?, phone=?, company=?, job_title=?, industry=?, address=?, city=?, country=?, source=?, status=?, assigned_to=?, user_id=?, notes=? WHERE id=?");
                $stmt->execute(array_values(array_merge($data, ['id' => $_POST['id']])));
                    ...
                    redirect('contacts.php', 'Contact updated successfully.', 'success');
                }
            } else {
                if (!$canAssign && $data['assigned_to'] != $userId) {
                    $error = 'You can only assign contacts to yourself.';
                } else {
                    $stmt = $db->prepare("INSERT INTO contacts (first_name, last_name, email, phone, company, job_title, industry, address, city, country, source, status, assigned_to, user_id, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    try {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            redirect('contacts.php', 'Contact deleted successfully.', 'success');
        } else {
...
<div class="lg:ml-64 min-h-screen transition-all duration-300">
    ...
    <main class="p-6 pt-20">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 animate-fade-in">
            <div>
                <h1 class="text-3xl font-bold text-secondary-900">Contacts</h1>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 animate-slide-up">
            <h2 class="text-xl font-bold text-secondary-900 mb-6"><?php echo $action === 'edit' ? 'Edit Contact' : 'Add New Contact'; ?></h2>
            <form method="POST" action="contacts.php" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 flex items-center gap-3">
                    <button type="submit" class="btn-primary px-6 py-2.5 text-white rounded-lg font-medium"><?php echo $action === 'edit' ? 'Update Contact' : 'Create Contact'; ?></button>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 animate-slide-up">
            ...
            <div class="overflow-x-auto">
                <?php if (empty($contacts)): ?>
                    <div class="py-12 text-center text-gray-500">No contacts found. <a href="contacts.php?action=add" class="text-primary-600 hover:underline">Create your first contact</a>.</div>
                <?php else: ?>
                <table class="w-full">
...
async function deleteAttachment(attachmentId, containerId) {
    ...
    try {
        const response = await fetch('api/attachment.php?id=' + attachmentId, { method: 'DELETE' });
        const data = await response.json();
        if (data.success) {
            window.location.reload();
        } else {
